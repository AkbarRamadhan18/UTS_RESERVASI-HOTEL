<?php
// Mengimpor file konfigurasi dan file fungsi 
require_once("config/config.php");
require_once("config/function.php");

//Mengatur zona waktu menjadi "Asia/Jakarta" dan membuat koneksi ke database.
date_default_timezone_set("Asia/Jakarta");
$thi = date('Y-m-d');
$connection = new Connection();
$conn = $connection->getConnection();

//Menghapus data reservasi sementara jika ada.
$sql2 = "DELETE FROM tmp_reservasi";
$delete2 = $conn->exec($sql2);

//Memeriksa apakah tombol "simpan" ditekan. Jika ya, maka mengambil nilai dari input form untuk nomor kamar, kelas, dan harga.
if (isset($_POST['simpan'])) {
  $nk = $_POST['nokamar'];
  $kelas = $_POST['kelas'];
  $harga = $_POST['harga'];

  // Memeriksa apakah nomor kamar sudah ada dalam database. Jika belum, maka akan menyimpan data kamar baru ke dalam database
  $sqlgetin6 = $conn->prepare("SELECT * FROM kamar WHERE no_kamar='$nk'");
  $sqlgetin6->execute();
  $ada = $sqlgetin6->fetch(PDO::FETCH_COLUMN) + 0;
  if ($ada == 0) {
    $sql = "INSERT INTO kamar VALUES ('$nk','$kelas','$harga')";
    $conn->exec($sql);


    echo "<script>alert(\"Data berhasil disimpan !\") ; window.location.href='?p=kamar' ;</script>";
  } else {
    echo "<script>alert(\"Data gagal disimpan, coba lagi !\") ; window.location.href='?p=kamar' ;</script>";
  }
}


?>
<!-- Mendefinisikan CSS dan JavaScript yang akan digunakan dalam tampilan. -->
<style>
  #frmCheckUsername {
    border-top: #F0F0F0 2px solid;
    background: #FAF8F8;
    padding: 10px;
  }

  .demoInputBox {
    padding: 7px;
    border: #F0F0F0 1px solid;
    border-radius: 4px;
  }

  .status-available {
    color: #2FC332;
  }

  .status-not-available {
    color: #D60202;
  }

  .dropdown-menu a {
    text-decoration: none;
    display: block;
    text-align: left;
  }

  #nou {
    text-decoration: none;
  }
</style>

<link href="assets/bs/css/bootstrap-datetimepicker.min.css" rel="stylesheet" media="screen">
<script>
  function checkAvailability() {
    jQuery.ajax({
      url: "cek_available2.php",
      data: 'nokamar=' + $("#nokamar").val(),
      type: "POST",
      success: function(data) {
        $("#user-availability-status").html(data);
      },
      error: function() {}
    });
  }
</script>
<div class="main-content">
  <div class="container-fluid">
    <h3><a href="?p=home">Beranda </a><i class="fa fa-angle-right"></i> <a href="?p=data_reservasi">Data Reservasi </a><i class="fa fa-angle-right"></i> Transaksi Reservasi</h3>

    <div class="row">
      <div class="col-lg-8">
        <div class="panel">
          <div class="panel-heading">
            <h4 class="mb"><strong> Pilih Tanggal Menginap</strong></h4>
          </div>
          <div class="panel-body">

            <!-- Membuat form untuk memilih tanggal menginap. Form ini mencakup input untuk tanggal "dari" dan "sampai". -->
            <form method="post" action="?p=trx_reservasi8" class="form-horizontal">

              <div class="form-group">
                <label class="col-sm-3 col-sm-3 control-label text-right">Dari Tanggal :</label>
                <div class="col-sm-9">

                  <!-- Menampilkan tombol "Cek Kamar" untuk melakukan pengecekan ketersediaan kamar berdasarkan tanggal yang dipilih. -->
                  <div class="input-group date ">
                    <input class="form-control" size="16" type="text" name="dari" required="required" oninvalid="this.setCustomValidity('Silahkan Masukan Tanggal !!!')" oninput="setCustomValidity('')" readonly="readonly" value="<?php echo $thi; ?>">
                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                  </div>
                </div>
              </div>
              <div class="form-group">
                <label class="col-sm-3 col-sm-3 control-label text-right">Sampai Tanggal :</label>
                <div class="col-sm-9">


                  <div class="input-group date form_date " data-date="" data-date-format="yyyy-mm-dd" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd">
                    <input class="form-control" size="16" type="text" name="sampai" required="required" oninvalid="this.setCustomValidity('Silahkan Masukan Tanggal !!!')" oninput="setCustomValidity('')" readonly="readonly">
                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                  </div>
                </div>
              </div>
              <div class="form-group text-right">
                <div class="col-sm-12">
                  <button class="btn btn-success" type="submit" name="btntanggal"> Cek Kamar</button>
                </div>
              </div>
            </form>
            <script type="text/javascript" src="assets/bs/jquery-3.2.1.min.js"></script>
            <script type="text/javascript" src="assets/bs/js/bootstrap.min.js"></script>
            <script type="text/javascript" src="assets/bs/js/bootstrap-datetimepicker.js" charset="UTF-8"></script>
            <script type="text/javascript" src="assets/bs/js/locales/bootstrap-datetimepicker.id.js" charset="UTF-8"></script>
            <script type="text/javascript">
              // Memuat script JavaScript yang digunakan untuk mengatur tampilan dan perilaku tanggal picker pada form.
              $('.form_datetime').datetimepicker({
                //language:  'fr',
                weekStart: 1,
                todayBtn: 1,
                autoclose: 1,
                todayHighlight: 1,
                startView: 2,
                forceParse: 0,
                showMeridian: 1
              });
              $('.form_date').datetimepicker({
                language: 'id',
                weekStart: 1,
                todayBtn: 1,
                autoclose: 1,
                todayHighlight: 1,
                startView: 2,
                minView: 2,
                forceParse: 0
              });
              $('.form_time').datetimepicker({
                language: 'id',
                weekStart: 1,
                todayBtn: 1,
                autoclose: 1,
                todayHighlight: 1,
                startView: 1,
                minView: 0,
                maxView: 1,
                forceParse: 0
              });
            </script>

          </div>

        </div>
      </div>
    </div><!-- col-lg-12-->
  </div><!-- /row -->
</div>
</div>